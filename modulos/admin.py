        ...
        if st.button("💾 Guardar todos los cambios", type="primary", use_container_width=True):
            with st.spinner("Guardando cambios..."):
                ...
                for idx, row in edited_df.iterrows():
                    ...
                    try:
                        # 1. Actualizar código si cambió (CORREGIDO)
                        materia_original = next((m for m in materias if m['id'] == materia_id), None)
                        if materia_original:
                            # Manejar valores nulos o float
                        ...
                        errores += 1
                        errores_lista.append(f"{materia_nombre}: {str(e)}")
                ...
                if errores == 0:
                    ...
                    for err in errores_lista[:5]:  # Mostrar solo los primeros 5 errores
                        st.error(f"❌ {err}")
...
def gestion_asignacion_academica(headers):
    ...
    st.caption("Asigna qué docente dicta cada materia en cada curso. Esto se usa al asignar horarios.")
    ...
    if curso_seleccionado:
        ...
        for m in materias:
            ...
            data.append({
                "id": m['id'],
                "Materia": m['nombre'],
                "Docente actual": docentes_dict.get(docente_id, "Sin asignar") if docente_id else "Sin asignar",
                "docente_id": docente_id
            })
