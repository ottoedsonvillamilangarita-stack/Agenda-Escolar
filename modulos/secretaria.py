...
def mostrar(data):
    ...
    secretaria = response.json()[0] if response.status_code == 200 else data
    ...
    usuario_completo = {
        "id_persona": secretaria["id_persona"],
        "nombre": secretaria["nombre"],
        "email": secretaria.get("email"),
        "telefono": secretaria.get("telefono"),
        "username": username
    }
    
    tab_perfil, tab_gestion = st.tabs(["👤 Mi Perfil", "📋 Gestión Académica"])
    ...
    with tab_gestion:
        ...
        st.info("Próximamente: carga masiva de estudiantes y docentes")
